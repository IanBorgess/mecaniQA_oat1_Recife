...
O relatório/apresentação completo da entrega está disponível em `mecaniQA_oat1_Recife.pdf`.
---
Índice
Contexto & Desafio
Arquitetura
...
Empacotamento (Docker) — build multi-stage da API Java, e containers oficiais para MySQL e Redis.
Orquestração local (Docker Compose) — comunicação via DNS interno entre serviços e persistência de dados.
Infraestrutura como Código (Terraform) — provisionamento declarativo e gestão de estado dos mesmos recursos Docker.
Camada de Controle (Kubernetes + K9s) — StatefulSets, Deployments, Services e observabilidade em tempo real.
Arquitetura
```
                     ┌─────────────────────┐
                     │      java-api        │
...
Docker e Docker Compose — para a Camada 1
Terraform `>= 1.x` — para a Camada 2
Minikube e kubectl — para a Camada 3
K9s (opcional) — para observabilidade do cluster Kubernetes
...
Documentação do Kubernetes — StatefulSets & Networking
Provider Docker no Terraform Registry
Repositório oficial do projeto: github.com/IanBorgess/mecaniQA_oat1_Recife
---
